...
public struct JumpEstimate has drop {
    source_gate_id: ID,
...
public fun new(
    ...
    let source_gate_base_fee = fta.gate_table()[object::id(source_gate)].current_fee(clock);
    let destination_gate_base_fee = fta
        .gate_table()[object::id(destination_gate)]
        .current_fee(clock);
