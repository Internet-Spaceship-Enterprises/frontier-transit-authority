...
public struct ManagementCap<phantom T> has key {
    id: UID,
    /// The ID of the object that the cap is for
...
public(package) fun transfer<T>(management_cap: ManagementCap<T>, new_owner: address) {
    transfer::transfer(management_cap, new_owner);
}

public(package) fun authorized_object_id<T>(management_cap: &ManagementCap<T>): ID {
    management_cap.authorized_object_id
}
